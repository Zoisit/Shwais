...
public class PlayerController2 : MonoBehaviour
{
    ...
    {        
        ...
        if (Input.GetKey(KeyCode.G) && lastdirection != DirectionEnum.west)
        {
            chosenDirection = new Vector2(1, 0);
            nextdirection = DirectionEnum.east;
        }
        if (Input.GetKey(KeyCode.J) && lastdirection != DirectionEnum.east)
        {
            chosenDirection = new Vector2(-1, 0);
            nextdirection = DirectionEnum.west;
        }
        if (Input.GetKey(KeyCode.Z) && lastdirection != DirectionEnum.south)
        {
            chosenDirection = new Vector2(0, 1);
            nextdirection = DirectionEnum.north;
        }
        if (Input.GetKey(KeyCode.H) && lastdirection != DirectionEnum.north)
        {
            chosenDirection = new Vector2(0, -1);
            nextdirection = DirectionEnum.south;
        }        
